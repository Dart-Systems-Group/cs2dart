...
3. WHEN no mapping is found for an `Attribute_Node`, THE `Attribute_Mapper` SHALL emit a `// UNMAPPED ATTRIBUTE: [<ShortName>(<args>)]` comment immediately before the annotated declaration and SHALL emit a `CG` `Warning` diagnostic carrying the `FullyQualifiedName`, the target declaration's source location, and a message indicating the attribute was not mapped. The `<args>` placeholder SHALL be serialized using the IR pretty-printed form: positional arguments are rendered as their IR expression pretty-print output separated by `, `; named arguments are rendered as `<Name>=<IR expression pretty-print>` separated by `, `; positional arguments appear before named arguments; if the attribute has no arguments the parentheses SHALL be omitted (e.g., `// UNMAPPED ATTRIBUTE: [Serializable]` rather than `// UNMAPPED ATTRIBUTE: [Serializable()]`).
