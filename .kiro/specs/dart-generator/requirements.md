...
- **Dart_Package**: The generated output for one `IrCompilationUnit`. Contains `ProjectName`, `OutputPath` (root directory), `Files` (list of `Gen_File`), `PubspecYaml` (the generated `pubspec.yaml` content), and `DependencyReportContent` (nullable string — the generated `dependency_report.md` content produced by the NuGet handler and carried through the pipeline; `null` when no NuGet packages were resolved).
