...
- **Load_Result**: The output of the `Project_Loader` consumed by the `Roslyn_Frontend`. Contains `Projects` (ordered list of `Project_Entry` items in topological order), `DependencyGraph`, `Diagnostics`, `Success`, and `Config`. Full schema is defined in the Project_Loader specification.
