...
- **Config_Service**: The `IConfigService` instance provided to the `Namespace_Mapper` at construction time; the sole source of all namespace mapping configuration values. The `Namespace_Mapper` SHALL NOT read `transpiler.yaml` directly.
...
4. WHERE barrel-file generation is enabled via `IConfigService.barrelFiles`, THE Namespace_Mapper SHALL generate an `export` statement in the parent library file for each direct child library.
...
3. WHEN a Conflict is detected and `IConfigService.namespaceMappings` provides no override and `IConfigService.autoResolveConflicts` is false, THE Namespace_Mapper SHALL halt code generation for the conflicting namespaces and continue processing non-conflicting namespaces.
4. WHEN `IConfigService.namespaceMappings` resolves a Conflict by assigning distinct Library_Paths to the conflicting namespaces, THE Namespace_Mapper SHALL use the overridden paths and emit no error.
5. WHEN `IConfigService.autoResolveConflicts` returns true and a Conflict is detected, THE Namespace_Mapper SHALL automatically generate a `namespace_mappings` entry for each conflicting namespace using the following deterministic procedure: sort the conflicting fully-qualified C# namespace strings lexicographically, assign the shared base Library_Path (without a suffix) to the first namespace in sorted order, and append `_2`, `_3`, … to the final path segment (before `.dart`) for each subsequent namespace in sorted order (e.g. `Acme.HttpClient` → `lib/acme/http_client.dart`, `Acme.HTTPClient` → `lib/acme/http_client_2.dart`). THE Namespace_Mapper SHALL emit a diagnostic warning listing the auto-generated mappings and write the resolved entries into a `transpiler.generated.yaml` file alongside `transpiler.yaml` so the developer can review and promote them to the main config.
6. WHEN `IConfigService.autoResolveConflicts` returns true and a `transpiler.generated.yaml` already contains a resolved entry for a conflicting namespace, THE Namespace_Mapper SHALL reuse that entry rather than generating a new one, ensuring stable output across repeated runs.
...
### Requirement 5: Consume the Configuration Service

**User Story:** As a pipeline module author, I want the `Namespace_Mapper` to receive all configuration values through `IConfigService`, so that it is decoupled from YAML parsing and file I/O.

#### Acceptance Criteria

1. THE Namespace_Mapper SHALL accept an `IConfigService` instance at construction time and SHALL use it as the sole source of all configuration values.
2. THE Namespace_Mapper SHALL NOT read or parse `transpiler.yaml` directly; all configuration access SHALL go through `IConfigService`.
3. WHEN `IConfigService.namespaceMappings` returns a non-empty map, THE Namespace_Mapper SHALL use the mapped Library_Path for any namespace that appears as a key.
4. WHEN `IConfigService.rootNamespace` returns a non-null value, THE Namespace_Mapper SHALL strip that prefix when computing Library_Paths.
5. WHEN `IConfigService.barrelFiles` returns true, THE Namespace_Mapper SHALL generate barrel export files for each namespace level.
6. WHEN `IConfigService.namespacePrefixAliases` returns a non-empty map, THE Namespace_Mapper SHALL apply the prefix replacements before snake_case conversion.
7. WHEN `IConfigService.autoResolveConflicts` returns true, THE Namespace_Mapper SHALL apply the auto-resolution procedure defined in Requirement 4.
8. WHEN a Library_Path value in `IConfigService.namespaceMappings` is invalid (e.g., contains uppercase letters or spaces), THE Namespace_Mapper SHALL emit a diagnostic error and fall back to the derived path.
9. WHEN all `IConfigService` accessors return their Default_Values, THE Namespace_Mapper SHALL apply all default transformation rules without error.
...
3. WHEN a C# `using` directive references a namespace that has an override in `IConfigService.namespaceMappings`, THE Namespace_Mapper SHALL use the overridden Library_Path in the generated `import` statement.
...
5. FOR ALL `IConfigService` instances that are valid, THE Namespace_Mapper SHALL produce output that is a deterministic function of (namespace string, `IConfigService`): same inputs always yield the same Library_Path.
